...
volatile uint8_t g_TfRxBuff[TF_RX_BUF_SIZE];
volatile uint32_t g_TfWriteCount;
volatile uint32_t g_TfReadCount;
...
bool _TF_FN TF_RemoveIdListener(TinyFrame *tf, TF_ID frame_id)
{
    ...
    TF_Error("ID listener %d to remove not found", (uint32_t)frame_id);
    return false;
}
...
bool _TF_FN TF_RemoveTypeListener(TinyFrame *tf, TF_TYPE type)
{
    ...
    TF_Error("Type listener %d to remove not found", (uint32_t)type);
    return false;
}
...
//    TF_Error("Unhandled message, type %d", (uint32_t)msg.type);
//}
...
bool _TF_FN TF_RenewIdListener(TinyFrame *tf, TF_ID id)
{
    ...
    TF_Error("Renew listener: not found (id %d)", (uint32_t)id);
    return false;
}
        ...
        case TFState_HEAD_CKSUM:
            COLLECT_NUMBER(tf->ref_cksum, TF_CKSUM) {
                ...
                if (tf->len > TF_MAX_PAYLOAD_RX) {
                    TF_Error("Rx payload too long: %d", (uint32_t)tf->len);
...
// use variables: si - signed uint32_t, b - byte, outbuff - target buffer, pos - count of bytes in buffer
...
{
    int8_t si = 0; // signed small uint32_t
    uint8_t b = 0;
    TF_ID id = 0;
    TF_CKSUM cksum = 0;
    uint32_t pos = 0;
...
void _TF_FN TF_Tick(TinyFrame *tf)
{
    ...
    for (i = 0; i < tf->count_id_lst; i++) {
        ...
        // count down...
        if (--lst->timeout == 0) {
            TF_Error("ID listener %d has expired", (uint32_t)lst->id);
